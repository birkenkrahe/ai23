...
"The desired workflow is one that has ChatGPT doing much of the
legwork, so a person only has to finalize tasks before completion."
#+caption: Image created by DALL-E 3 using Bing chat from this prompt
#+attr_latex: :width 400px
[[../img/chatgpt_workflow.jpeg]]

- Design experiment for tech use to decide about ChatGPT use.
- Use the use case framework to decide if ChatGPT adoption is safe.
